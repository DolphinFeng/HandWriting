<template>
    <div class="category-tree-container">
        <mtd-cascader
            ...
            :placeholder="$getText('category_tree_placeholder', '请选择/搜索服务目录')"
            ...
            filterable
            clearable
            :no-data-text="$getText('category_tree_no_data', '暂无发起权限')"
...
export default class CategoryTree extends Vue {
    @Prop()
    ctiTree: CommonTypes.ctiTreeItem[];
    ...
    async setDefaultCatalog () {
        let formatLabel = this.formatLabelOptions([this.categoryName, this.typeName, this.itemName]);
    ...
    handleTreeData (currentArray, slugArr, level) {
        ...
        currentArray.forEach((item) => {
            ...
            // 对最末层的rgId单独处理
            if (item.rgId) {
                item.rgName && (obj.label += '（' + item.rgName + '）');
                obj.value = {
    ...
    async categoryOptionChange (val, options, manual) {
        ...
        if (this.categoryInfo.categoryName === '找不到合适的目录') {
            ...
            this.categoryInfo.rgId = this.env === 'prod' ? 342 : 1702;
