...
export default class SearchTagDropdown extends Vue {
    ...
    async createTicketTags (name) {
        ...
        try {
            ...
            // this.$mtd.message.error(`${this.$getText('search_tag_dropdown_tag_create_failed', '标签创建失败，原因')}：${e}`);
            console.log(e);
        }
    ...
    get errorTip () {
        ...
        if (getStringLength(this.targetTag) > 20) {
            error = this.$getText('search_tag_dropdown_tag_name_length_limit', '标签名称不能超过20个字符');
        } else {
            error = '';
        }
        return error;
