    ...
    <div class="ticket-helpdesk">
        <div class="ticket-helpdesk-rg-container ticket-helpdesk__item">
            <div class="ticket-helpdesk__content">
                ...
                <div
                    class="load-more"
                    @click="getRgCustomTemplate"
                    v-if="templateList.length < total">{{ $getText('ticket_helpdesk_rg_load_more', '加载更多') }}</div>
            </div>
        </div>
        <div class="ticket-helpdesk_funcbar ticket-helpdesk__item">
            <div class="ticket-helpdesk__content" v-if="bulletin && bulletin.enable">
                <h2 class="funcbar-title"><i class="mtdicon mtdicon-bell-o" /><span>{{ $getText('ticket_helpdesk_rg_bulletin_title', '最新公告') }}</span></h2>
                <p class="funcbar-content">
                    {{ bulletin.content }}
                </p>
            </div>
            <div class="ticket-helpdesk__content" v-if="faqList.length">
                <h2 class="funcbar-title"><i class="mtdicon mtdicon-question-circle-o" /><span>{{ $getText('ticket_helpdesk_rg_faq_title', '常见问题') }}</span></h2>
 ...
 */
@Component
export default class TicketHelpdeskRg extends PaginationMixin {
    @Getter spaceDomain;
    ...
    async getHelpdeskBulletins () {
        const res = await api.ctiApi.getHelpdeskBulletins({
            rgId: this.rgId * 1,
            enable: true,
            sn: 1,
            cn: 1
    ...
    async getRgCustomTemplate () {
        ...
        try {
            const res: Ajax.AxiosResponse = await api.ticketApi.getRgCustomTemplate({
                cn: this.currentPage,
                sn: this.limit,
                rgId: this.rgId
            });
