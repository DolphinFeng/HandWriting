...
function computeSlotSegPressures(seg) {
  ...
  if (seg.forwardPressure === null) {
    ...
    for (i = 0; i < forwardSegs.length; i++) {
      ...
      // 子元素就一层
      forwardPressure = Math.max(forwardPressure, 1 + (forwardSeg.forwardPressure as number))
    }
...
function computeSegForwardBack(
  ...
  if (seg.forwardCoord === null) {
    ...
    seg.backwardCoord =
      seg.forwardCoord - (seg.forwardCoord - seriesBackwardCoord) / (seriesBackwardPressure + 1)
...
export function computeDateTop(target: number, startDate: Date): number {
  const targeTime = new Date(target)
  let hour = targeTime.getHours()
  let minute = targeTime.getMinutes()
  // 如果日期不是今天，小于今天就从0开始，大于今天就是24点结束
  if (targeTime.toDateString() !== startDate.toDateString()) {
...
export function computeSegVerticals(segs: IEventItem[], dayDate: Date, eventMinHeight: number = 0) {
  for (const seg of segs) {
