...
export class ImageExtend {
    ...
    pasteHandle(e) {
        ...
        if (clipboardData) {
            ...
            if (item && item.kind === 'file' && item.type.match(/^image\//i)) {
                ...
                let self = this
                let size = self && self.file && self.file.size;
                const configSize = self.config.size
                // 如果图片限制大小
                if (configSize && size && size >= configSize * 1024 * 1024) {
                    if (self.config.sizeError) {
                        self.config.sizeError()
                    }
                    return
                }
                if (this.config.action && self.file) {
                    setTimeout(() => {
                        this.uploadImg(true)
                    }, 0)
                } else {
    ...
    uploadImg(fromPaste = false) {
        ...
        xhr.upload.onprogress = function (e) {
            // TODO: 加上进度后，粘贴图片时编辑器会失焦，导致无法获取到当前焦点
            if (!fromPaste) {
    ...
    uploadError(xhr) {
        let res = JSON.parse(xhr.responseText);
        let errText = res.data && res.data.errorMsg ? '图片上传失败：' + res.data.errorMsg : '[upload error]';
        QuillWatch.active.quill.root.innerHTML
...
export function imgHandler() {
    ...
    if (fileInput === null) {
        ...
        // 限制可选图片类型
        fileInput.setAttribute('accept', 'image/png,image/gif,image/jpg,image/jpeg,image/svg');
...
export function getImageUrls (fileGetData) {
    ...
    let imgs = fileGetData.match(imgReg);
    let imgSrcs = imgs.map(img => {
        let imgItem = img.match(srcReg);
        return imgItem[1];
    });
    return imgSrcs;
}
