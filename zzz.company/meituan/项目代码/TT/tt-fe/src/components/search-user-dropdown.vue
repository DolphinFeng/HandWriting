...
export default class SearchUserDropdown extends Vue {
    ...
    async remoteMethod (query) {
        ...
        try {
            const res: Ajax.AxiosResponse = await api.ctiApi.searchUser({ keyword: query, includeVirtual: true, ticketId: this.ticketId });
            this.userList = res.data.items.map(item => {
                let matchStr = item.i18nDisplayName ? `${item.i18nDisplayName}/${item.username}` : (item.displayName ? `${item.displayName}/${item.username}` : item.username);
                return {
                    username: item.username,
                    displayName: item.displayName,
                    i18nDisplayName: item.i18nDisplayName,
