<template>
    <div>
        <mtd-radio-group
            ...
            @input="typeChange"
            :disabled="disabled">
            <mtd-radio value="MEITUAN">{{ $getText('component_relation_poi_type_meituan', '美团门店ID') }}</mtd-radio>
            <mtd-radio value="DIANPING">{{ $getText('component_relation_poi_type_dianping', '点评门店ID') }}</mtd-radio>
        </mtd-radio-group>
        <div>
            <mtd-input
                v-model="shopId"
                :placeholder="$getText('component_relation_shop_id_placeholder', '请填写对应门店ID')"
                @blur="getPoiInfo"
                style="width:100%;"
                :disabled="disabled" />
