    ...
    <div
        ...
        <mtd-dropdown
            ...
            <mtd-dropdown-menu
                ...
                <mtd-input
                    v-model="targetTag"
                    :placeholder="placeholder || $getText('search_tag_dropdown_new_placeholder', '搜索标签')"
                    @input="debounceRemoteMethod"
                    @keydown.enter="enterOperate"
                    class="search-tag-input"
                    prefix-icon="mtdicon mtdicon-search"
                    clearable />
                <div class="tags-title" v-if="!((!targetTag) && (!recommandTagList.length))">{{ `${!targetTag ? $getText('search_tag_dropdown_new_common_tags', '常用标签') : $getText('search_tag_dropdown_new_search_result', '搜索结果')}：` }}</div>
                ...
                <div class="no-match-tip" v-if="noMatchTag && !errorTip">{{ $getText('search_tag_dropdown_new_no_match', {tagName: targetTag}) }}</div>
                <div class="no-match-tip error-message" v-if="errorTip && noMatchTag"> {{ errorTip }} </div>
                <div class="no-match-tag" v-if="noMatchTag">
                    <mtd-button
                        type="text-primary"
                        @click="createTag"
                        :loading="buttonLoading">{{ $getText('search_tag_dropdown_new_create_tag', '创建标签') }}</mtd-button>
                </div>
            </mtd-dropdown-menu>
        </mtd-dropdown>
    </div>
...
export default class SearchTagDropdown extends Vue {
    ...
    get rgId () {
        return parseInt(this.$route.params.rgId, 10) || 0;
    }
