...
import {
  getTicketByName,
  getCustomFormList,
  getTicketListNumberFromSpace,
  getSpaceInfoFromDomain,
  getTemplateListFromSpaceId,
  getTicketListNumber,
...
import imgAsk from '@images/home_ask.png'

// "createdAt": 1597302734557,
// 		"accessLink": "luoan.tt.cloud.test.sankuai.com",
// 		"createdBy": "luoan",
// 		"formListLink": "luoan.tt.cloud.test.sankuai.com/custom/form/list",
// 		"name": "骆安的空间",
// 		"id": 24,
// 		"orgId": 19897
...
class HomeSpace extends Component<IProps, IState> {
  _rgId = 0
  private pageSize = 20
  ...
  imgListHome2 = [imgCreate, imgToDo]

  constructor(props) {
    super(props)

    let type = this.props?.screenProps?.extra2 ?? 'list'

    this.state = {
      isSearching: false,
      searchKeyWord: '',
      spaceId: null,
      // spaceId: '24',  // FIXME: Temp use
      displayType: type,
    }
  ...
  async requestSpaceInfo() {
    const spaceName = this.props?.screenProps?.extra
    ...
    if (spaceName?.length > 0) {
      ...
      if (r?.code === 200 && r?.data != null) {
        ...
        this.setState({ spaceId: spaceId, spaceModel: r?.data })

        if (spaceId.length > 0) {
          this.requestListNumber(spaceId)
          ;(this._listRef as MWSList)?._onRefresh()
        }
      } else {
        Toast.open(
          r?.message ?? i18nClient.t('components_home_a2168d', { defaultValue: '请求出错' }),
        )
      }
    }
  }

  async requestTempalteList() {
    const r = await getTemplateListFromSpaceId(this.state.spaceId, 'CUSTOM', 1, 10)

    if (r?.code === 200 && r?.data?.items != null) {
      // this.setState({ spaceId: r.data?.id ?? '' })
    } else {
      Toast.open(r?.message ?? i18nClient.t('components_home_a2168d', { defaultValue: '请求出错' }))
    }
  }
    ...
    return (
      <View style={{ backgroundColor: 'white', flex: 1 }}>
        ...
        <View style={this.state.isSearching ? { display: 'none' } : { display: 'flex', flex: 1 }}>
          {this.renderImage()}
          {this.renderEntries()}
          {this.state.displayType === 'home' ? null : this.renderList()}
  ...
  renderEntryItem = (index: number) => {
    ...
    const pageList =
      this.state.displayType === 'home' ? this.SpaceHomeButtonList : this.SpaceListButtonList
    const type = pageList[index]

    const imgW = 20
    const list = this.state.displayType === 'home' ? this.imgListHome : this.imgListHome2
    return (
      <TouchableHighlight
      ...
      >
        ...
        >
          <Image source={list[index]} style={{ width: imgW, height: imgW }} />
  ...
  async newOnLoadEntryList(pageSize: number, pageNo: number, refresh: boolean) {
    ...
    if (this.state.spaceId) {
      const res = await getTemplateListFromSpaceId(this.state.spaceId, 'CUSTOM', 1, 10)
      ...
      console.log('items here 11', res)

      const items: Array<any> = res?.data?.data?.items
  ...
  getNameFromPageType(t: TTHomePageType) {
    ...
    switch (t) {
      ...
      case TTHomePageType.faqi:
        return i18nClient.t('components_home_01c85e', { defaultValue: '我要提问' })
    }
  }
  ...
  goToCreateFromSpace(spaceModel?: any) {
    console.log('goto create from space', spaceModel)
  ...
  handleItemClick = (pageType: TTHomePageType) => {
    ...
    if (pageType === TTHomePageType.created) {
      ...
      this.goToMyTodoList()
    } else if (pageType === TTHomePageType.faqi) {
      this.goToCreateFromSpace(this.state.spaceModel)
    }
  }
