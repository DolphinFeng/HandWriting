...
const emit = defineEmits(['update:visible', 'created', 'changeSize', 'changePos']);
...
const property = reactive({
  ...
  panelMove(ev) {
    ...
    let titleBox = document.getElementsByClassName('title-box');
    let panelBox = document.getElementsByClassName('panel-box');
    if (property.left <= 0) {
      property.left = 0;
    }
    // if(property.left >= viewportWidth - titleBox[0].offsetWidth){
    //   property.left = viewportWidth - titleBox[0].offsetWidth
    // }
    if (property.left >= viewportWidth - 200) {
      property.left = viewportWidth - 200;
    }

    if (property.top - titleBox[0].offsetHeight <= 0) {
      property.top = titleBox[0].offsetHeight;
    }
    // if(property.top >= viewportHeight - panelBox[1].offsetHeight){
    //   property.top = viewportHeight - panelBox[1].offsetHeight
    // }
    if (property.top >= viewportHeight - titleBox[0].offsetHeight) {
      property.top = viewportHeight - titleBox[0].offsetHeight;
    }
    emit('changePos', property.left, property.top);
  },
  panelUp() {
...
(function () {
  ...
  //都是-1, 默认居中
  if (props.left == -1 && props.top == -1) {
    property.left = (bodyWidth - property.width) / 2;
    property.top = (bodyHeight - property.height) / 2;
  }
