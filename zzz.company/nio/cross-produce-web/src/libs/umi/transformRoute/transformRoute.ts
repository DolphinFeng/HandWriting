...
function formatter(props: FormatterProps, parent: Partial<MenuDataItem> = {path: '/'}): MenuDataItem[] {
  ...
  return data
    ...
    .map((item = {path: '/'}) => {
      ...
      const item_pro_layout_parentKeys = new Set([...pro_layout_parentKeys, ...(item.parentKeys || [])]);
...
const defaultFilterMenuData = (menuData: MenuDataItem[] = []): MenuDataItem[] =>
  menuData
    .filter(
      (item: MenuDataItem) => item && (item.name || notNullArray(item.children)) && !item.hideInMenu && !item.redirect,
    )
