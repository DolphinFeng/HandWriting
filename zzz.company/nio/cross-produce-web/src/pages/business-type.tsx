...
export const BusinessType = ({
  ...
  //   try {
  //     const modelResult: any = await cpmService.getBatchModeList();

  //     let result = modelResult.data.map((item: any) => ({
  //       label: item.name,
  //       value: item.value,
  //     }));
  //     if (modeDisabledType == 'vector') {
  //       result.forEach((element: any, index: any) => {
  //         if (element.value == 1) {
  //           result[index]['disabled'] = true;
  //         }
  //       });
  //     } else if (modeDisabledType == 'avp') {
  //       result.forEach((element: any, index: any) => {
  //         if (element.value == 2) {
  //           result[index]['disabled'] = true;
  //         }
  ...
  return (
    <>
      <Form.Item label="生产模式" name="businessType" rules={[{required}]}>
        <Select
          placeholder="请选择"
          style={{width: '200px'}}
          allowClear
