 ...
 * @return {*}  {number}
 */
export function getTileIDFromLatLng(lng, lat, level) {
 ...
 * @return {*}  {number}
 */
export function getTileIDByNdsCoord(ndsCoord, level) {
 ...
 * @return {*}  {ILngLatCoord}
 */
export function getLngLatFromTileID(tileID, level) {
 ...
 * @return {*}  {ILngLatCoord}
 */
export function getLngLatFromTileNum(tileNum) {
  const tileArr = tileNum.split('');
  let lngStr = ''; let latStr = '';
  ...
  const lngResult = getLngFromNdsCoord(parseInt(getNdsCoordFromNegative(lngStr), 2));
  const latResult = getLatFromNdsCoord(parseInt(getNdsCoordFromNegative(latStr), 2));
 ...
 * @return {*}  {boolean}
 */
function judgementNdsCoordPositive(ndsCoord) {
  return ndsCoord[0] === '0';
}
 ...
 * @return {*}  {string}
 */
function getNdsCoordFromNegative(ndsCoord) {
  // 首数字为0则证明在正轴 可以直接返回
 ...
 * @return {*}  {string}
 */
export function getTileNum(tileID, level) {
  const _tileID = typeof tileID === 'number' ? tileID.toString(2) : parseInt(tileID, 10).toString(2); // 转成二进制
  return _tileID.substring(16 - level).padEnd(63, '0');
}

/**
 * @description 根据二进制NDS坐标取经度
 * @export
 * @param {number} lngNdsCoord
 * @return {*}  {number}
 */
export function getLngFromNdsCoord(lngNdsCoord) {
  return lngNdsCoord * 360 / (2 ** 32);
}

/**
 * @description 根据二进制NDS坐标取纬度
 * @export
 * @param {number} latNdsCoord
 * @return {*}  {number}
 */
export function getLatFromNdsCoord(latNdsCoord) {
  return latNdsCoord * 180 / (2 ** 31);
}
 ...
 * @return {*}  {number}
 */
export function getLngTileBoundarySizeFromLevel(level) {
  return 360 / (2 ** (level + 1));
}

/**
 * @description 纬度范围内求该瓦片层级下每格tile的 size
 * @export
 * @param {number} level
 * @return {*}  {number}
 */
export function getLatTileBoundarySizeFromLevel(level) {
  return 180 / (2 ** level);
}
 ...
 * @return {*}  {number}
 */
export function getLatNdsCoord(lat) {
  return Math.floor((lat / 180) * (2 ** 31));
}
 ...
 * @return {*}  {number}
 */
export function getLngNdsCoord(lng) {
  return Math.floor((lng / 360) * (2 ** 32));
}
 ...
 * @return {*}
 */
export function ndsCoordToString(ndsCoord, type) {
  const codeLength = type === 'x' ? 32 : 31;

  // 如果经纬度为负则取补码
  if (ndsCoord < 0) {
    const originalCode = Math.abs(ndsCoord).toString(2)
      .padStart(codeLength, '0'); // 求原码
 ...
 */
function toNdsCoordinates(lng, lat) {
  return {
    lng_nds: ndsCoordToString(getLngNdsCoord(lng), 'x'),
    lat_nds: ndsCoordToString(getLatNdsCoord(lat), 'y'),
  };
}
 ...
 */
function getMortonCodes({ lng_nds, lat_nds }) {
  let str = '';
 ...
 * @return {*}  {string}
 */
function getNegateFromOriginalCode(originalCode) {
  return originalCode.split('').map(code => (code === '0' ? '1' : '0'))
    .join('');
}
 ...
 * @return {*}  {string}
 */
function addBinary(code1, code2) {
  let result = '';
  let carry = 0;
  for (let i = code1.length - 1, j = code2.length - 1; i >= 0 || j >= 0; i -= 1, j -= 1) {
    let sum = carry;
 ...
 * @return {*}  {string}
 */
export function subBinary(code1, code2) {
  // 二进制数
...
export function tileID2Coords(tileId, level) {
  const tile = parseInt((0xFFFFFFFF / Math.pow(2, (31 - 2 * level)))) & tileId;
  const code = tile * Math.pow(2, (62 - 2 * level));
  const x = CompactBits(code);
  const y = SignedBitRestore(parseInt(x / 2));
  console.log(x, y);
}

function CompactBits(morton) {
  let temp_val = morton;
  temp_val &= 0x5555555555555555;
  temp_val = (temp_val ^ (parseInt(temp_val / 2))) & 0x3333333333333333;
  temp_val = (temp_val ^ (parseInt(temp_val / Math.pow(2, 2)))) & 0x0F0F0F0F0F0F0F0F;
  temp_val = (temp_val ^ (parseInt(temp_val / Math.pow(2, 4)))) & 0x00FF00FF00FF00FF;
  temp_val = (temp_val ^ (parseInt(temp_val / Math.pow(2, 8)))) & 0x0000FFFF0000FFFF;
  temp_val = (temp_val ^ (parseInt(temp_val / Math.pow(2, 16)))) & 0x00000000FFFFFFFF;
  return temp_val;
}

function SignedBitRestore(val) {
  return val & 0x40000000 ? val | 0x80000000 : val;
}
