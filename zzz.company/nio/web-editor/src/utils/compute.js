...
import {Cartesian2, Cartesian3, Cartographic, Cesium3DTileFeature, Math as CMath} from "cesium";
...
export function getUserPos() {
    ...
    if (pos) {
        pos = pos.split(',');
        return new Cartesian3(parseFloat(pos[0]), parseFloat(pos[1]), parseFloat(pos[2]));
    } else {
        //给予用户一个默认位置
...
export function searchDrillPicks(stPos, edPos) {
    ...
    let width = Math.abs(edPos.x - stPos.x), height = Math.abs(edPos.y - stPos.y);
    ...
    return viewer.scene.drillPick(winPos, 1000, Math.round(width) * 2, Math.round(height) * 2);
}

/**
 * 搜索任意类型的一条odd或lane并返回，odd优先级较高
 * @param pos{Cartesian2}
 * @return {OddLane|null}
 */
export function searchOnePick(pos) {
    let picks = viewer.scene.drillPick(pos, 10, 10), target = null;
    //优先返回oddLane
    for (let i = 0; i < picks.length; i++) {
        if (picks[i].id instanceof OddLane) {
            //target = picks[i].id;
            target = picks[i];
            break;
        }
        if (picks[i] instanceof Cesium3DTileFeature) {
            target = picks[i];
        }
    }
    return target;
...
export function getScreenPoint(x, y, type = 'degrees') {
    let pos = viewer.camera.pickEllipsoid(new Cartesian2(x, y), viewer.scene.globe.ellipsoid);
