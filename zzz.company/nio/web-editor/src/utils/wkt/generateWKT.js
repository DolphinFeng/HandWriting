    ...
    stringify: {
        linestring: function (positions) {
            positions = positions.map(item => {
                ...
                return `${CMath.toDegrees(cartographic.longitude)} ${CMath.toDegrees(cartographic.latitude)} ${cartographic.height}`;
            });
            return `LINESTRING Z(${positions.join(',')})`;
        },
        point: function (position) {
            let cartographic = Cartographic.fromCartesian(position);
            return `POINT Z(${CMath.toDegrees(cartographic.longitude)} ${CMath.toDegrees(cartographic.latitude)} 0)`;
        }
    }
}
