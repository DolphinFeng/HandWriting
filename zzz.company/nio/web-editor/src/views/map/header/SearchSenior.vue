    ...
    >
      ...
      <template #default>
        <el-tabs type="border-card">
          ...
          <!-- WKT图层 -->
          <el-tab-pane label="按事件ID">
            <div class="add-item">
              <div class="add-item-content">
                <input
                  v-model="searchData.event_id"
                  :class="{'apply-shake': searchData.shake_id}"
                  class="add-item-input"
                  type="text"
                  placeholder="事件ID"
            ...
            <div class="add-item" style="justify-content: flex-end">
              <el-button type="primary" @click="eventIdHandler">
                <span>确定</span>
              </el-button>
            </div>
          </el-tab-pane>
          <!-- Issue 图层 -->
          <el-tab-pane label="按Issue ID">
            <div class="add-item">
              <div class="add-item-content">
                <input
                  v-model="searchData.issue_id"
                  :class="{'apply-shake': searchData.shake_id}"
                  class="add-item-input"
                  type="text"
            ...
            <div class="add-item" style="justify-content: flex-end">
              <el-button type="primary" @click="issueIdHandler"> 确定 </el-button>
            </div>
          </el-tab-pane>
        </el-tabs>
      </template>
...
import {parseWKT} from '../../../utils/wkt/parseWKT';
import {searchSeniorPanelVisible, setSearchSeniorPanelVisible} from '../../../system/searchSenior.js';
import {fetchLink, fetchSdLink} from './SearchHdLink.js';
import {createHighlightLineEntity, clearAllTempEntities} from '../../../system/tempEntities.js';
import {issueLayer} from '../../../system/issue/layer.ts';
import {getBaseMapUrl} from '../../../system/layer/tileLayer/tileLayerController.js';
...
async function checkIssueId() {
  if (searchData.issue_id === '') {
    return Promise.reject('err');
  }
...
const eventIdHandler = function () {
  let event_id = searchData.event_id;
  ...
  checkEventId()
    .then((res) => {
      ...
      //左移32位
      axios
        .post(apiDynamicURL + '/dynamic-map/event/master/get', {
          eventId: event_id,
          mapVersion: store.state.version.curVersion,
        })
        .then((res) => {
          if (res.status !== 200 || res.data === null || res.data.data === null || res.data.data.length === 0) {
            NioMessage('warning', '未找到该事件id结果，请核实id或版本号', 1500);
            return false;
          }

          let geometry_wkt = res.data.data.geometry;
          let coordinates = parseWKT.read(geometry_wkt);

          createHighlightLineEntity(coordinates);
        })
        .catch((err) => {
          NioMessage('error', '查询事件id数据失败:' + err.message, 1500);
          return false;
        })
        .finally(() => {
          searchData.loading = false;
...
const issueIdHandler = () => {
  let issue_id = searchData.issue_id;
  issue_id = issue_id.trim();
  checkIssueId()
    .then((res) => {
      searchData.loading = true;
      ...
      //左移32位
      let flag = axios
        .post(issueServiceURL + '/nio/material/issue/list-odd', {
          ndsVsn: store.state.version.curVersion,
          issueUuids: [issue_id],
        })
        .then((res) => {
          if (res.status !== 200 || !res.data || !res.data.length) {
            NioMessage('warning', '未找到该id结果，请核实id或版本号', 1500);
            return false;
          }

          const issue = res.data[0];

          issueLayer.showSearchedIssue(issue);
        })
        .catch((err) => {
          NioMessage('error', '查询事件id数据失败:' + err.message, 1500);
          return false;
