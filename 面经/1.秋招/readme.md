...
# 水滴一面 11.13 （全程 60min）
>> 这个面试官问的八股会结合开发情景
## 既然写项目时碰到了最大并发数为 6 的限制，为什么没看控制台，应该也可以看出来
## 性能优化说几个
## performance 看过吗，火焰图，加载时序
## 如何统计用户表现行为，浏览器 performance 这个api
## 如何保证需求稳定上线
    答案：需求评审，单元测试，集成测试，端到端测试，持续集成，回归测试，用户验收测试，性能测试，监控和日志
    追问：需求评审，正序，逆序啥的
## 用户白屏但是监控日志看不到会有什么样的情况
    答案：静态资源还没加载出来，项目在用户的编译阶段就报错（为什么开发看不到，因为用户的系统不同）
## 深浅拷贝开发中用过吗
    答案：表单处理的时候，用户编辑表单，万一用户取消编辑，就需要把深拷贝的原始数据给还原
## 301,302,400, 401, 403 状态码遇到过吗
    答案：301 是永久重定向，302 是临时重定向，400 遇到过，但是自己的语法没问题，是后端状态码不规范
## webpack, vite 源码有看过吗
## 对函数式编程的理解
## 美团是否要求 ai 代码率，对 新技术 的了解，对 ai 的看法，纯聊天
## 未来规划
## event-loop，这个设计的目的
## 监控日志原理
    答案：前端监控日志，主要就是捕获错误，捕获错误的方式有三种，一种是 window.onerror，一种是 promise 的 catch，一种是 try catch
## 指出我的不足
...
## 不同域名的 cookie 能否共享
    答案：不能，跨域了
## 什么情况下才需要拆包
    答案：当一个页面需要加载的资源超过 10 个，并且每个资源的大小超过 100kb 时，就需要拆包

# 水滴三面 11.19 （全程 30min）
## 设计模式了解吗
    追问：工厂模式和抽象工厂模式的区别
## java 的垃圾回收机制和 js 的垃圾回收机制的区别
    答案：java 是基于标记清除，js 是基于标记整理
## 线上报警原理
## tcp 四层模型是哪四层
